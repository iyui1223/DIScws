 ...
 */
 ...
 private:
     ...
     Array2D<double> temperature;
     Array2D<double> nextTemperature;
     ...
     void saveFrame(int frameNumber) {
         ...
         outFile.write(reinterpret_cast<const char*>(temperature.getPointer()), temperature.getSize() * sizeof(double));
         outFile.close();
             ...
             for (int x = 1; x < width - 1; x+=block_size) {
                ...
                for (int yy = y; yy < lim_y; yy++){
                    for (int xx = x; xx < lim_x; xx++){
                        double laplacian =
                            temperature(yy + 1, xx) + temperature(yy - 1, xx) +
                            temperature(yy, xx + 1) + temperature(yy, xx - 1) -
                            4.0 * temperature(yy, xx);
    
                        nextTemperature(yy, xx) = temperature(yy, xx) + diffusionRate * laplacian;                       
                    }    
                }
             }
         }
