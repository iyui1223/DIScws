...
# Optional peek into checkpointed LoRA params
if args.resume_training:
    lora_path = f"{exp_name}.pt"
    print(f"\n[Debug] Loading LoRA state from: {exp_name}.pt")
    loaded_state = torch.load(lora_path, map_location="cpu")
    print("[Debug] Top 5 parameters in checkpoint:")
    print(loaded_state)
    for k, v in list(loaded_state.items())[:5]:
        print(f"{k}: shape={v.shape}, mean={v.float().mean():.4f}, std={v.float().std():.4f}")

# Peek at model's current LoRA parameters
print("\n[Debug] Current model's top 5 trainable LoRA parameters:")
model_state = model.state_dict()
trainable_lora_params = {k: v for k, v in model_state.items() if v.requires_grad}
for k, v in list(trainable_lora_params.items())[:5]:
    print(f"{k}: shape={v.shape}, mean={v.float().mean():.4f}, std={v.float().std():.4f}")
    ...
    for (batch,) in progress_bar:
        ...
        if steps % eval_every == 0 or steps == 1:
            ...
            if val_loss < best_val_loss:
                ...
                lora_state_dict = {
                    k: v for k, v in model_to_save.state_dict().items() if v.requires_grad
                }
